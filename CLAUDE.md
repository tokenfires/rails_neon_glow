...
8. **Grep for existing `::before`/`::after` rules before adding new pseudo-elements.** `.ng-border-glow` uses BOTH pseudo-elements (::before for gradient border, ::after for blur glow). Phase 4 stomped ::before (VFD labels invisible). Phase 5a stomped ::after (Retrowave cards lost their glow). When you need to layer a visual on an element that might carry `.ng-border-glow`, use `background-image` instead of a pseudo-element — it layers on top of `background-color` with zero collisions.
7. **Don't let palette-specific reasoning contaminate palette-agnostic infrastructure.** When a universal utility like `.ng-neon-tube` behaves differently under one palette, it stops being a utility. Each palette should carry its identity through ITS OWN scopes (palette-prefixed selectors, palette-specific utility classes), not by carving exceptions into shared rules. If you find yourself writing `.neon-foo .ng-utility { ... }` to "fix" how the utility looks on that palette, stop — you're either (a) reaching for a utility that doesn't actually fit that palette, in which case use a different tool, or (b) the palette identity already lives in its own scopes and doesn't need the override. I've made this mistake three times on a single branch; the fix is always "remove the override, trust the palette's own scopes to carry its identity."
